    ...
    {	
        ...
        for ($i=0; $i <$end ; $i++) { 
            ...
            $query = DB::table('transaksi_detail')
                    ...
                    ->select(DB::raw("SELECT(SUM(biaya)*80/100)-if((SELECT SUM(jumlah) FROM pencairan_dana WHERE STATUS = 'berhasil' AND mitra_id = '$mitra_id'),0) total,(SELECT SUM(jumlah) FROM pencairan_dana WHERE STATUS = 'berhasil' AND MONTH(created_at) = '$month_number') AS penarikan"))
                    ->get();
